...
async function placeOrder() {
  let orderToPlace;
  if (!current?.error && current.find((e) => e.tableID == selectedTableID)) {
    // already existing occupation
    orderToPlace = JSON.stringify({ orders: newOrder }); //todo: get waiterID from auth
    putData(
      `api/occupations/placeOrder/${
        current.find((e) => e.tableID == selectedTableID)._id
      }`,
      orderToPlace
    );
  } else {
    //new occupation
    orderToPlace = JSON.stringify({
      tableID: selectedTableID,
      waiterID: "64569b00b931d3131de2403e",
      orders: newOrder,
    }); //todo: get waiterID from auth
...
async function checkout() {
  ...
  putData(
    `api/occupations/${current.find((e) => e.tableID == selectedTableID)._id}`,
    JSON.stringify({ checkOutTime: checkoutTime })
  );
...
function renderNewOrder() {
  ...
  commentTextboxes.forEach((textbox) => {
    textbox.addEventListener("change", (textbox) => {
      ...
      addOrRemoveComment(
        textbox.currentTarget.dataset.itemid,
        textbox.currentTarget.dataset.instanceid,
        textbox.currentTarget.value
      );
    });
  });
}
